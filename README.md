...
[Apple Music](https://www.apple.com/apple-music/) is Apple's music streaming service that provides access to millions of songs, curated playlists, and personalized recommendations, along with exclusive content and live radio stations.
...
3. Click the "+" button to create a new key, then check the "MusicKit" checkbox under Key Services.

4. Enter a key name and click Continue, then click Register to generate your private key.

5. Download the private key file (.p8) and note your Key ID and Team ID, as you'll need all three for API authentication.
...
musicUserToken = "<Your_Music_User_Token>"
```

2. Create an `apple.music:ApiKeysConfig` and initialize the client:

```ballerina
configurable string authorization = ?;
configurable string musicUserToken = ?;

final music:Client appleMusicClient = check new({
    authorization,
    musicUserToken
});
```
...
public function main() returns error? {
    check appleMusicClient->/me/library.post({
        ids: ["songs:123456789", "songs:987654321"]
    });
}
```
...
1. [Music discovery workflow](https://github.com/ballerina-platform/module-ballerinax-apple.music/tree/main/examples/music-discovery-workflow) - Demonstrates how to implement automated music discovery and recommendation workflows using the Apple Music connector.
2. [Music collection analytics](https://github.com/ballerina-platform/module-ballerinax-apple.music/tree/main/examples/music-collection-analytics) - Illustrates analyzing and generating insights from music collections and listening data.
