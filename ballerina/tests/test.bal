...
configurable string authorization = isLiveServer ? os:getEnv("APPLE_MUSIC_AUTHORIZATION") : "test_jwt_token";
...
isolated function testGetMultipleCatalogArtists() returns error? {
    ArtistsResponse response = check appleMusicClient->/catalog/["us"]/artists(ids = ["909253", "136975"]);
    test:assertTrue(response.data.length() > 0, "Expected a non-empty artists array");
}
...
isolated function testSearchCatalog() returns error? {
    SearchResponse response = check appleMusicClient->/catalog/["us"]/search(term = "billie+eilish");
...
isolated function testGetMultipleCatalogSongs() returns error? {
    SongsResponse response = check appleMusicClient->/catalog/["us"]/songs(ids = ["1440857784"]);
...
isolated function testGetCatalogSong() returns error? {
    SongsResponse response = check appleMusicClient->/catalog/["us"]/songs/["1708308989"]();
    test:assertTrue(response.data.length() > 0, "Expected a non-empty songs array");
}
...
isolated function testGetCatalogSongRelationship() returns error? {
    SongsResponse response = check appleMusicClient->/catalog/["us"]/songs/["1708308989"]/["albums"]();
    test:assertTrue(response.data.length() > 0, "Expected a non-empty songs array");
}
...
isolated function testGetLibraryAlbum() returns error? {
    LibraryAlbumsResponse response = check appleMusicClient->/me/library/albums/["i.1441164670"]();
    test:assertTrue(response.data.length() > 0, "Expected a non-empty library albums array");
}
...
isolated function testGetLibraryAlbumRelationship() returns error? {
    LibraryArtistsResponse response = check appleMusicClient->/me/library/albums/["i.1441164670"]/["artists"]();
...
isolated function testGetLibraryArtist() returns error? {
    LibraryArtistsResponse response = check appleMusicClient->/me/library/artists/["i.159260351"]();
    test:assertTrue(response.data.length() > 0, "Expected a non-empty library artists array");
}
...
isolated function testGetLibraryArtistRelationship() returns error? {
    LibraryAlbumsResponse response = check appleMusicClient->/me/library/artists/["i.159260351"]/["albums"]();
    test:assertTrue(response.data.length() > 0, "Expected a non-empty library albums array");
}
...
isolated function testGetLibrarySong() returns error? {
    LibrarySongsResponse response = check appleMusicClient->/me/library/songs/["i.1708308989"]();
    test:assertTrue(response.data.length() > 0, "Expected a non-empty library songs array");
}
...
isolated function testGetLibrarySongRelationship() returns error? {
    LibrarySongsResponse response = check appleMusicClient->/me/library/songs/["i.1708308989"]/["albums"]();
    test:assertTrue(response.data.length() > 0, "Expected a non-empty library songs array");
}
